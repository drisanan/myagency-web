...
"[project]/services/commitsScraper.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function cleanName(raw) {
    return raw.replace(/video/gi, '').replace(/scouts? report/gi, '').replace(/\|/g, '').replace(/\s{2,}/g, ' ').trim();
}
function parseSchoolFromHref(href) {
    if (!href) return '';
    const parts = href.split('/').filter(Boolean);
    const last = parts[parts.length - 1] || '';
    return last.replace(/-/g, ' ').replace(/\b\w/g, (m)=>m.toUpperCase()).trim();
}
function extractImgParam(src) {
    if (!src) return '';
...
async function scrapeESPNBasketballClass(year) {
...
async function scrapeBasketballTop() {
    const classes = [
        '2026',
        '2027'
    ];
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8ea2b221._.js.map