...
var handler = async (event) => {
  ...
  try {
    ...
    if (!res.ok) {
      const errMsg = data && typeof data === "object" && "message" in data ? data.message : "Lookup failed";
      return { statusCode: res.status, headers, body: JSON.stringify({ ok: false, error: errMsg }) };
    }
    const contact = data && typeof data === "object" && Array.isArray(data.contacts) ? data.contacts[0] : void 0;
    if (!contact) {
      return { statusCode: 404, headers, body: JSON.stringify({ ok: false, error: "Contact not found" }) };
    }
    const customFields = contact.customField || [];
    const accessField = customFields.find((f3) => f3.id === accessCodeFieldId);
    ...
    let agencyId = (customFields.find((f3) => f3.id === agencyIdFieldId)?.value || "").toString().trim();
    const agencyName = (customFields.find((f3) => f3.id === agencyNameFieldId)?.value || "").toString().trim();
    const agencyColor = (customFields.find((f3) => f3.id === agencyColorFieldId)?.value || "").toString().trim();
    const agencyLogo = (customFields.find((f3) => f3.id === agencyLogoFieldId)?.value || "").toString().trim();
    const isNew = agencyId === "READY";
    let resolvedAgencyId = agencyId;
    if (isNew) {
      console.log("Agency ID is READY. Initializing new agency record...");
      const newAgencyId = `agency-${(0, import_crypto.randomUUID)()}`;
      try {
      ...
      if (contact.id) {
        try {
          const updateBody = {
            customField: customFields.map(
              (f3) => f3.id === agencyIdFieldId ? { ...f3, value: resolvedAgencyId } : f3
            )
          };
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        agency: {
          id: resolvedAgencyId,
          // This is now the Real UUID, never "READY"
          name: agencyName || void 0,
          color: agencyColor || void 0,
          logoUrl: agencyLogo || void 0,
          isNew: false
          // Always false now because we just handled the "new" case
        }
      })
    };
  } catch (e2) {
    console.error("ghl-login lambda error", e2);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ ok: false, error: e2?.message || "Server error" })
    };
  }
};
