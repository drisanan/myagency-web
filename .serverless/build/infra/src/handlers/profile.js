...
var Response = class _Response extends Body {
  ...
  static error() {
    const response2 = new _Response(null, { status: 0, statusText: "" });
...
var handler = async (event) => {
  ...
  const contactId = session.contactId || session.userId;
  if (!contactId) {
    return badRequest(origin, "No contact ID in session");
  }
  try {
    const updatePayload = {};
    if (firstName !== void 0) updatePayload.firstName = firstName;
    if (lastName !== void 0) updatePayload.lastName = lastName;
    if (accessCode !== void 0) {
    ...
    const updated = await ghlRes.json();
    return response(200, {
      ok: true,
      profile: {
